<?php
  ...
  # insert keyid, fingerprint, time, the current time and counter as 1 into the secrets table, save counter in last_insert_id, if row exists, update it by increasing the counter by one and assign counter to last_insert_id
  define("MYSQL_WRITE", "INSERT INTO secrets (keyid, fingerprint, time, counter) VALUES (?, ?, CURRENT_TIMESTAMP, LAST_INSERT_ID(1)) ON DUPLICATE KEY UPDATE counter = LAST_INSERT_ID(counter + 1)");
