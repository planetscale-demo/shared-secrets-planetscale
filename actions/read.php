      ...
      if (!empty($secret)) {
        ...
        if (0 < count($recipients)) {
          ...
          if (null !== $decrypted_secret) {
            $link = mysqli_init();
            # enforce SSL if MYSQL_USER is set to non empty value
            if (defined("MYSQL_USER") && !empty(MYSQL_USER)) {
                ...
                if ($statement = mysqli_prepare($link, MYSQL_WRITE)) {
                  ...
                  if (mysqli_stmt_bind_param($statement, "ss", $keyid, $fingerprint)) {
                    if (mysqli_stmt_execute($statement)) {
                      # save last insert id in last_insert_id
                      $last_insert_id = mysqli_insert_id($link);

                      if (1 === $last_insert_id) {
                        $result = $decrypted_secret;
                      } else {
                        $error = "Secret was already requested $last_insert_id times";
                      }
                    } else {
                      if (DEBUG_MODE) {
